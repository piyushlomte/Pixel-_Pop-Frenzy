class Game {
    constructor() {
        ...
        // Set initial state of Start Game button
        if (this.startGameButton) {
            this.startGameButton.disabled = true;
            this.startGameButton.style.opacity = '0.5';
            this.startGameButton.style.cursor = 'not-allowed';
        }
        ...
        // Initialize wallet and player data
    ...
    createBackgroundSound() {
        return {
            play: () => {
                if (this.backgroundOscillators.length > 0) return;
                ...
                const playMelody = () => {
                    melody.forEach((note, index) => {
                        ...
                        this.backgroundGainNodes.push(gainNode);
                ...
                this.backgroundOscillators.push(bassOscillator);
                this.backgroundGainNodes.push(bassGain);
                ...
                this.backgroundOscillators.push(backgroundOscillator);
                this.backgroundGainNodes.push(backgroundGain);
    ...
    drawGameOver() {
        ...
        // Only show coins earned if player beat the top score
        const coinsEarnedElement = document.getElementById('coinsEarned');
        if (this.coinsEarned > 0) {
            coinsEarnedElement.textContent = this.coinsEarned;
            coinsEarnedElement.style.display = 'block';
        } else {
            coinsEarnedElement.style.display = 'none';
        }
    ...
    updateWalletDisplay() {
        ...
        if (walletBalance) {
            walletBalance.textContent = this.playerData.totalCoins || 0;
        }
    }
    ...
    setupFrontPage() {
        ...
        this.userNameInput.addEventListener('input', () => {
            this.playerName = this.userNameInput.value.trim();
        ...
        this.startGameButton.addEventListener('click', () => {
            if (this.playerName && this.walletConnected) {
                this.showGamePage();
            }
        });
    }
    ...
    setupWalletControls() {
        ...
        if (!walletStatus || !walletControls) {
    ...
    updateWalletUI() {
        ...
        if (this.walletConnected && this.walletAddress) {
            ...
            // No wallet connected - hide the controls
    ...
    handleDisconnect() {
        ...
        // Reset player data but keep the player name
        this.playerData = {
        ...
        this.walletStatus.style.color = '#ff4444';
    ...
    connectWallet() {
        ...
        this.walletStatus.style.color = '#00ff00';
        ...
        // Load player data for this wallet
        this.loadPlayerData();
    }
    ...
    updateStartButton() {
        ...
        this.startGameButton.disabled = !(this.playerName && hasValidAddress);
    }
    ...
    showGamePage() {
        ...
        // Update player stats with the user's name
        this.updatePlayerStats();
    ...
    updatePlayerStats() {
        ...
        if (statsContainer) {
            statsContainer.innerHTML = `
                ...
                ${this.playerData.isVerified ? '<p class="verified">✓ Verified Account</p>' : ''}
            `;
        }
    }
    ...
    async savePlayerData() {
        ...
        try {
            ...
            if (response.ok) {
                ...
                // Only show reward message if player beat the top score
                if (data.beatTopScore) {
                    this.coinsEarned = data.coinsEarned;
                    this.showRewardMessage(data.coinsEarned);
                } else {
                    this.coinsEarned = 0;
                }
            }
    ...
    showRewardMessage(coinsEarned) {
        if (coinsEarned > 0) {
            const rewardMessage = document.createElement('div');
            rewardMessage.className = 'reward-message';
            rewardMessage.innerHTML = `
                <h3>🎉 New High Score! 🎉</h3>
                <p>You beat the top score and earned 1 coin!</p>
                <p>Your coin has been added to your wallet.</p>
            `;
            
            document.querySelector('.game-over').appendChild(rewardMessage);
